...
let resource =
    (
      ...
      ~edit=not_found_id,
      ~show=not_found_id,
      ~update=_ => not_found_id,
      ~destroy=not_found_id,
    ) => {
  open Header.CacheControl;
  let no_store = Filter.cache_control(No_store);
  fun
  | (`GET, [] | [""]) => index
...
let string_of_unix_addr =
  fun
  | Unix.ADDR_UNIX(string) => string
  | [@implicit_arity] Unix.ADDR_INET(inet_addr, _) =>
...
let websocket_handler = (handler, resolver, _, wsd) => {
  ...
  let (incoming, queue_incoming) = Lwt_stream.create();
  let eof = () => {
    Wsd.close(wsd);
    resolve();
  };

  let frame = (~opcode, ~is_fin as _, bigstring, ~off, ~len) =>
    switch (opcode) {
    | `Continuation
    | `Text
    | `Binary =>
      queue_incoming(
        Some(Ok(Bigstringaf.substring(~off, ~len, bigstring))),
      )
    | `Connection_close =>
  ...
  let pull = timeout_s => {
    ...
    Lwt.pick([
      msg,
      timeout_s |> Lwt_unix.sleep |> Lwt.map @@ (() => Error(`Timeout)),
    ]);
  };
  ...
  let push = string => {
    ...
    let bigstring = Bigstringaf.of_string(~off, ~len, string);
...
let upgrade_handler = (addr, upgrade, handler, ()) => {
  ...
  ws_conn |> Gluten.make((module Websocketaf.Server_connection)) |> upgrade;
...
let error_handler = (_, ~request as _=?, error, handle) => {
  let message =
    switch (error) {
  ...
  let body = handle(H.Headers.empty);
...
let filter = (server, route) =>
  (
    if (Config.Default.Filters.csp) {
      [] |> Header.ContentSecurityPolicy.make |> Filter.csp;
    } else {
      Fun.id;
    }
  ) @@
  (
    if (Config.Default.Filters.hsts) {
      two_years |> Header.StrictTransportSecurity.make |> Filter.hsts;
    } else {
      Fun.id;
    }
  ) @@
...
let serve = (~port, server) => {
  let request_handler = (client_addr, {Gluten.Reqd.reqd, upgrade}) => {
    let f = () => {
      let send =
        fun
        | `HTTP(resp, body) => {
            let code = H.Status.to_code(resp.H.Response.status);
            client_addr
            |> string_of_unix_addr
            |> Printf.printf(" %d %s\n%!", code);

            let writer = H.Reqd.respond_with_streaming(reqd, resp);
            let result = Piaf.Body.iter(schedule_chunk(writer), body);
            Lwt.on_success(
              result,
              fun
              ...
              | Error(error) => {
                  Reqd.report_exn(
                    reqd,
                    Failure(Piaf.Error.to_string(error)),
                  );
        ...
        | `WebSocket(headers, handler) => {
            ...
            switch (
              Websocketaf.Handshake.respond_with_upgrade(
                ...
                ~sha1,
                reqd,
                upgrade_handler(client_addr, upgrade, handler),
              )
            ) {
            | Ok () => ()
            | Error(string) =>
              let headers = H.Headers.of_list([("connection", "close")]);
              let response = H.Response.create(~headers, `Bad_request);
              Reqd.respond_with_string(reqd, response, string);
            };
          };

      let (meth, path, query) = reqd |> H.Reqd.request |> parse_route;
      let server = filter(server);
      let response = reqd |> Request.make(query) |> server((meth, path));
  ...
  let listen_addr = Unix.([@implicit_arity] ADDR_INET(inet_addr_any, port));
  open Lwt.Syntax;
  let* _ =
...
let serve = (~port=Config.Default.port, server) =>
  server |> serve(~port) |> Lwt_main.run;
